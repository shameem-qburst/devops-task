                    ...
                    docker.image('sonarsource/sonar-scanner-cli').inside {
                        stage('Sonarqube') {
                            withCredentials([string(credentialsId: 'SONAR_TOKEN', variable: 'SONAR_TOKEN')]) {
                                sh 'sonar-scanner -Dsonar.projectKey=$SONAR_PROJECT_KEY -Dsonar.sources=Kubernetes,Terraform,django_todo -Dsonar.host.url=$SONAR_URL -Dsonar.token=$SONAR_TOKEN'
                            }
                        }
                    }
                }
